...
if ($this->settings_info["hide_shop_name_on_invoice"] == 0) {
    echo "            <tr>\n                <td style=\"text-align: center;font-size: 18px; font-weight: bold;line-height: 22px;\">\n                    ";
...
for ($i = 0; $i < count($data["items"]); $i++) {
    ...
    if ($data["currency_system_default"] == 2) {
        if (0 < $data["items"][$i]["discount"]) {
            ...
            $unit_price = $data["items"][$i]["selling_price"] * $data["invoice"][0]["rate"];
        }
    } else {
        if (0 < $data["items"][$i]["discount"]) {
            $unit_price = $data["items"][$i]["selling_price"] * (1 - $data["items"][$i]["discount"] / 100) * $data["invoice"][0]["rate"];
    ...
    echo "Total:<span style='float:right'>" . self::value_format_custom_no_currency($total, $this->settings_info) . "</span>";
    echo "</td>\n            </tr>\n            \n            ";
