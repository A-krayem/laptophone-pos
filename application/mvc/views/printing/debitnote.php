    ...
    <body class="A4">
        ...
        <div class="row" style="margin-top: 20px;border-top: 1px solid">
            <div class="col-xs-6" style="margin-top: 10px;">
                <b>To: </b> <?php if(!is_null($data["supplier"])) echo ucwords ($data["supplier"][0]["name"])." ".ucwords ($data["supplier"][0]["middle_name"])." ".ucwords ($data["supplier"][0]["last_name"]); ?><br/>
                <b>Address: </b> <?php if(!is_null($data["supplier"][0]["address"])) echo ucwords ($data["supplier"][0]["address"]); ?><br/>
                <?php if(!is_null($data["supplier"][0]["phone"]) && $data["supplier"][0]["phone"]!="") echo "<b>PHONE: </b>".ucwords ($data["supplier"][0]["phone"]); ?>
        ...
        <table class="inv_table" style="margin-top: 5px;">
            ...
            <?php 
                ...
                for ($i = 0; $i < count($data["returned"]); $i++) { 
                    $total+=$data["returned"][$i]["price"]*$data["returned"][$i]["returned_debit"];
                ?>
            <tr>
                <td style="font-size: 15px;width: 100px;"><?php echo self::idFormat_item($data["returned"][$i]["item_id"]); ?></td>
                <td style="font-size: 15px;"><?php echo $data["items"][$data["returned"][$i]["item_id"]]["description"]; ?></td>
                <td style="font-size: 15px;"><?php echo number_format($data["returned"][$i]["price"],3); ?></td>
                <td style="font-size: 15px;text-align: center"><?php echo floatval($data["returned"][$i]["returned_debit"]); ?></td>
                <td style="font-size: 15px;"><?php echo number_format($data["returned"][$i]["price"]*$data["returned"][$i]["returned_debit"],2); ?></td>
            </tr>
            <?php } ?>
            <tr>
                <td style="font-size: 15px; border: none !important" colspan="2">&nbsp;</td>
                <td style="font-size: 15px; font-weight: bold" colspan="2">Total Amount</td>
                <td style="font-size: 15px;font-weight: bold"><?php echo number_format($total,2); ?></td>
            </tr>
            <?php if($data["currency_system_default"]==1){ ?>
            <tr>
                <td style="font-size: 15px; border: none !important" colspan="3">&nbsp;</td>
                <td style="font-size: 15px; font-weight: bold">Total in (LBP)</td>
                <td style="font-size: 15px;font-weight: bold"><?php echo number_format($total,2); ?></td>
            </tr>
            <?php } ?>
            <tr>
