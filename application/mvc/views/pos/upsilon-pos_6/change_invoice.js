...
function calculate_finale_prince_inv_changed(id){
    $("#total_inv_ch_"+id).html(accounting.formatMoney($("#qty_"+id).val()*$("#price_inv_ch_"+id).val()*(1-$("#disc_inv_ch_"+id).val()/100)*parseFloat(1+$("#vat_inv_ch_"+id).html()/100), { symbol: default_currency_symbol,  format: "%v %s" }));
    get_total_of_new_items();
    get_difference();
}
...
function discounts_history(id){
    ...
    $('#history_discount_Modal').on('shown.bs.modal', function (e) {
        ...
        $('#quick_stock_table__').on('key-focus.dt', function(e, datatable, cell){
            //$(history_discount_table__.row(cell.index().row).node()).addClass('selected');
        });

        $('#quick_stock_table__').on('key-blur.dt', function(e, datatable, cell){
            //$(history_discount_table__.row(cell.index().row).node()).removeClass('selected');
        });
    });
